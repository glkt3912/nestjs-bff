...
BFF（Backend for Frontend）とは、フロントエンド専用に最適化されたバックエンドサービスです。
...
BFFがあれば、フロントからの1回のリクエストをトリガーに BFF が複数バックエンドをまとめて呼び出し、
...
フロントは BFF との通信に Cookie のみを使えばよく、
...
BFFがあれば、BFF を先にデプロイして新旧バックエンドの差異を吸収することも、
...
このリポジトリは NestJS で BFF を実装しており、採用理由は以下のとおりです。
